...
{
	struct s5pv210_nand *nand = (struct s5pv210_nand *)samsung_get_base_nand();
	printf("s5pv210_nand_enable_hwecc(%p, %d)\n", mtd, mode);
				      ...
				      u_char *ecc_code)
{
	...
	printf("s5pv210_nand_calculate_hwecc(%p,):\n"
...
{
	u32 cfg = 0;
	// u-boot-2014.04/arch/arm/include/asm/arch-s5pc1xx/cpu.h
	struct s5pv210_nand *nand_reg = (struct s5pv210_nand *)(struct s5pv210_nand *)samsung_get_base_nand();

	printf("board_nand_init: struct s5pv210_nand *nand_reg =  %p\n", (struct s5pv210_nand *)samsung_get_base_nand());
#if 0
	/* initialize hardware */
	...
	/* initialize hardware */
	/* HCLK_PSYS=133MHz(7.5ns) */
	cfg =	(0x3 << 23) |	/* Disable 1-bit and 4-bit ECC */
			/* 下面3个时间参数稍微比计算出的值大些（我这里依次加1），否则读写不稳定 */
			(0x4 << 12) |	/* 7.5ns * 2 > 12ns tALS tCLS */
			(0x3 << 8) | 	/* (1+1) * 7.5ns > 12ns (tWP)  TWRPH0*/
			(0x2 << 4) | 	/* (0+1) * 7.5 > 5ns (tCLH/tALH)  TWRPH1 */

			(0x1 << 3) | 	/* K9GAG08U0F: MLC NAND Flash */
			(0x1 << 2) |	/* 2KBytes/Page */
			(0x1 << 1);		/* 5 address cycle */
#endif
